		...
		<xsl:for-each select="JobSearch/Jobs/Job[substitute_xpath_here]">
			<div class="JobSearchDetail ui-corner-all" >
				...
				<div class="JobSearchDetailInfo" >
					...
					<p><label>Contact:</label><a class="JobSearchDetail" href="mailto:{contactemail}?subject=Job Ref. {ourref}"><xsl:value-of select="contactname"/></a><span id="LoginStrip{JobId}"><span> , </span><a class="JobSearchDetail" id="Login{JobId}" onClick="apply('Login')"  >Log in</a><span> , or </span><a class="JobSearchDetail" id="Register{JobId}" onClick="apply('Register')" >Register</a></span></p>
