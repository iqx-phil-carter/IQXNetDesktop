...
function IQXIsValidBankSortCode(n){								// Valid Bank Sort number - 99-99-99
...
// postcode format drawn from www2.royalmail.com
function IQXIsValidPostCodeTwo(n){							// Valid Post Code - two character outbound
	return /^[a-zA-Z][1-9] [0-9][a-zA-Z][a-zA-Z]$/.test(n);
	}

function IQXIsValidPostCodeThree(n){						// Valid Post Code - three character outbound
...
// additional formatting for New Zealand
function IQXIsValidNZIRDNumber(n){							// Valid IRD Number - New Zealand NI Number
	return /^[1-9][1-9][1-9]{0,1}-[1-9][1-9][1-9]-[1-9][1-9][1-9]$/.test(n);
	}

function IQXIsValidNZBankSortCode(n){								// Valid sort code - New Zealand Tax Code
	return /^[0-9][0-9]-[0-9][0-9]-[0-9][0-9]$/.test(n);
	}

function IQXIsValidNZBankAccount(n){							// Valid Bank account Code - New Zealand Tax Code
	return /^[0-9][0-9][0-9][0-9][0-9][0-9][0-9]-[0-9][0-9][0-9]$/.test(n);
	}

function IQXIsValidNZCombinedBankAccount(n){						// Valid Full 18 digit Bank account Code - New Zealand Tax Code
	return /^[0-9][0-9]-[0-9][0-9][0-9][0-9]-[0-9][0-9][0-9][0-9][0-9][0-9][0-9]-[0-9][0-9][0-9]$/.test(n);
	}
	
function IQXIsValidNZTaxCode(n){								// Valid Tax Code - New Zealand Tax Code
	return /^[A-Z]{0,5}$/.test(n);
	}
	
function IQXIsValidFourDigitTime(n){								// Valid four digit time - 9999
	return /^[0-9][0-9][0-9][0-9]$/.test(n);
	}

//additional formatting for New Zealand

function IQXIsValidNumeric(n) {						// Valid numeric value
    if (n <= 0) { return false; }
    return $.isNumeric(n);
	}
	

function dataTable_summaryRowChecker(dataRow,summaryCols,testCol,iTestCol) {
	// testCol is either a col number which is the summary column we are looking for (position in the summaryCols array), or 'detail' 
	// returns false if the row is to be hidden
	for (var i = 0; i < summaryCols.length; i++) {
		if (testCol == 'detail' || iTestCol == i) {
			if (dataRow[summaryCols[i][1]]=='') return false;
		}
		else 
			if (iTestCol != NaN && iTestCol != i) {
				if (dataRow[summaryCols[i][1]]!='') return false;
			}
		}
	return true;
...
function dataTable_grandTotalRow(dataRow,summaryCols) {
	// returns true if the row is the grand total row
	for (var i = 0; i < summaryCols.length; i++) {
		if (dataRow[summaryCols[i][1]]!='') return false;
		}
	return true;
...
(function($){  
  ...
  $.validator.addMethod("time", function(value, element) { 
		...
		}, "Please enter a valid 24 hour time, e.g. 01:00");
  ...
  $.validator.addMethod("validBankSortCode", function(value, element) {
		return this.optional(element) || (IQXIsValidBankSortCode(value) ); 
		}, "Please use '99-99-99' format for Sort Code");
  
  $.validator.addMethod("validBankAccountNumber", function(value, element) {
		return this.optional(element) || (IQXIsValidBankAccount(value) ); 
...
//additional formatting for New Zealand
  $.validator.addMethod("validNZIRDNumber", function(value, element) {
		return this.optional(element) || (IQXIsValidNZIRDNumber(value) ); 
		}, "Please use '123-123-123' format for IRD Number");
  
  $.validator.addMethod("validNZBankSortCode", function(value, element) {
		return this.optional(element) || (IQXIsValidNZBankSortCode(value) ); 
		}, "Please use 12 3456 for the Bank and Branch Number");
  
  $.validator.addMethod("validNZBankAccountNo", function(value, element) {
		return this.optional(element) || (IQXIsValidNZBankAccount(value) ); 
		}, "Please use 1234567-123 for Bank Account numbers");
  
  $.validator.addMethod("validNZCombinedBankAccountNo", function(value, element) {
		return this.optional(element) || (IQXIsValidNZCombinedBankAccount(value) ); 
		}, "Please use 12-1234-1234567-123 for Bank Account numbers");
		
  $.validator.addMethod("validNZTaxCode", function(value, element) {
		return this.optional(element) || (IQXIsValidNZTaxCode(value) ); 
		}, "Please use XXXXX for Tax Codes");
  
    $.validator.addMethod("validFourDigitTime", function(value, element) {
		return this.optional(element) || (IQXIsValidFourDigitTime(value) ); 
		}, "Please use 9999");
		
//additional formatting for New Zealand
 
    $.validator.addMethod("IQXIsValidNumeric", function (value, element) {
        return IQXIsValidNumeric(value);
  ...
  $.fn.rowafter = function (options) {
	...
	$.fn.dataTableExt.aoFeatures.push( {
    	"fnInit": function( oSettings ) {
			...
			if (!theData) {   //typeof(theData) == 'undefined') {
				return null;
			}
	...
	$.fn.dataTableExt.aoFeatures.push( {
    	"fnInit": function( oSettings ) {
			var theData=$(oSettings.nTable).data('IQXdtSummaryFilter');
			if (!theData) {   
				return null;
			}
			var theColumns=theData.columns;
			var theHTML = '<div class="dataTables_summaryFilter">'
			   +'Summarise Data <select>'
			   +'<option value="">No</option>';
			var i=0;
			for (i=0;i<theColumns.length;i++){
				if (theColumns[i][0]){
					theHTML+='<option value="'+i+'">By '+theColumns[i][0]+'</option>';
				}
			}
			theHTML+='</select></div>';
			var theDiv=$(theHTML).get(0);
			var theCombo=$('select',theDiv).get(0);
			$(theCombo).change(function(){oSettings.oApi._fnReDraw(oSettings);});
			theData.combo=theCombo;
			return theDiv;
    		},
    	"cFeature": "S",
    	"sFeature": "SummaryFilter"
		} );
	...
	$.fn.dataTableExt.afnFiltering.push(
		function( oSettings, aData, iDataIndex ) {
			...
			var sFilterVal=$(theData.combo).val();
			if (sFilterVal=='') sFilterVal='detail';
			var iFilterVal=parseInt(sFilterVal);
			return dataTable_summaryRowChecker(aData,theData.columns,sFilterVal,iFilterVal);
	}
	);

	// End of dataTables features
