...
function TableTools ( oInit )
{
	...
	function fnFeaturePrint ()
	{
		...
		$(nButton).click( function() {
			...
			_DTSettings._iDisplayStart = 0;   // MHS fix - if they weren't on the top page it did not show all rows
	...
	function fnPrintHideNodes( nNode )
	{
		...
		{
			if ( nChildren[i] != nNode && nChildren[i].nodeType == 1 && !($(nChildren[i]).hasClass('printable')))   // MHS added hasClass to allow items outside the table to be included in the print
			{
